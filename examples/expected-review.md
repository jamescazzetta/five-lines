...
8 changed method(s) reviewed · 9 finding(s) · 1 Jev request(s)
...
- `web/cart.ts:15` — `shippingCost` branches with if/else between two pieces of its own domain logic _(Jev 0.68; worth a look)_
...
- `web/cart.ts:22` — a condition in `drain` has a side effect _(Jev 0.94; introduced)_
...
- `web/cart.ts:10` — `getTotal` is an accessor that invites callers to do the object's work _(Jev 0.76; worth a look)_
